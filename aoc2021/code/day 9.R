...
find_local_minima <- function(mat){
  ...
  combined <-
  rbind(input[1:(rows-1),] < input[2:rows,], rep(TRUE,rows)) &
  rbind(rep(TRUE,rows), input[2:rows,] < input[1:(rows-1),]) &
  cbind(input[,1:(cols-1)] < input[,2:cols], rep(TRUE,cols)) &
  cbind(rep(TRUE,cols), input[,2:cols] < input[,1:(cols-1)])
